...
public class RecoveryController {
    ...
    // GET /pi/recovery/patient/{patientId}/score-forecast?tenantId=1&days=3
    // Linear regression forecast: predicts where the return score is heading
    // over the next N days based on all existing check-ins.
    // Minimum 2 check-ins required for a meaningful regression.
    @GetMapping("/patient/{patientId}/score-forecast")
    public ResponseEntity<ScoreForecastResponse> getScoreForecast(
            @PathVariable Long patientId,
            @RequestParam Long tenantId,
            @RequestParam(defaultValue = "3") int days) {
        return ResponseEntity.ok(service.getScoreForecast(patientId, tenantId, days));
    }
