...
@Tag(name = "User Management", description = """
        ...
        ## User Roles
        - **PATIENT**: Regular patients who can view their own profile
        - **DOCTOR**: Medical staff who can view patient records within their hospital
        - **ADMIN**: System administrators with full access to all user data
...
public class UserController {
    ...
    @Operation(
            ...
            description = """
                    ...
                    This endpoint validates the user's credentials and returns JWT tokens
                    if authentication is successful. The response includes an access token
                    ...
                    ## Response Details
                    - Access token: JWT token for API authentication (expires based on app.jwt.expiration-ms)
                    - Refresh token: JWT token for obtaining new access tokens (valid for 7 days)
                    - User ID: The authenticated user's unique identifier
                    - Role: The user's role in the system (PATIENT, DOCTOR, or ADMIN)
                    """)
    @ApiResponses(value = {
    ...
    @Operation(
            ...
            description = """
                    Allows new patients to register themselves in the system.

                    This is a public endpoint that does not require authentication.
                    Patients can create their own account with their personal information.

                    ## Request Details
                    - Email must be unique across the system (not already registered)
                    - Password must be at least 6 characters
                    - tenantId is required to specify which organization/hospital

                    ## Response Details
                    - Returns the created patient profile
                    - Does NOT return JWT tokens (patient must login after registration)

                    ## Post-Registration
                    After successful registration, the patient should use the login
                    endpoint to authenticate and receive JWT tokens.
                    """)
    @ApiResponses(value = {
    ...
    @Operation(
            ...
            description = """
                    ...
                    The refresh token must be valid and not revoked/blacklisted.

                    ## Request Details
                    - Refresh token must be a valid JWT token previously issued by the system
                    - Token must not be blacklisted (e.g., from logout)

                    ## Response Details
                    - New access token with updated expiration
                    - Same refresh token (not rotated)
                    - User ID and role remain the same
                    """)
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Token refreshed successfully. Returns new access token.",
                    content = @Content(mediaType = "application/json")),
    ...
    @Operation(
            ...
            description = """
                    ...
                    This endpoint invalidates the refresh token by adding it to a blacklist,
                    preventing its future use for token refresh. The access token remains valid
                    ...
                    ## Response Details
                    - Returns 200 OK regardless of token validity (idempotent operation)
                    """)
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Logout successful. Token has been blacklisted.",
                    content = @Content(mediaType = "application/json")),
            @ApiResponse(responseCode = "401", description = "Invalid refresh token format.",
                    content = @Content(mediaType = "application/json")),
            @ApiResponse(responseCode = "400", description = "Bad request. Invalid input format.",
                    content = @Content(mediaType = "application/json"))
    })
    ...
    @Operation(
            ...
            description = """
                    ...
                    This endpoint returns the user details for the currently logged-in user
                    ...
                    - Includes patient-specific fields (medical record number, blood type, etc.) if applicable
                    """)
    @ApiResponses(value = {
    ...
    @Operation(
            ...
            description = """
                    ...
                    This endpoint returns all non-deleted users. Only users with ADMIN role
                    can access this endpoint.

                    ## Response Details
                    - Returns list of all active users
                    - Each user includes their profile, role, and tenant information
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "401", description = "Unauthorized. Invalid or missing authentication token.",
                    content = @Content(mediaType = "application/json")),
            @ApiResponse(responseCode = "403", description = "Forbidden. Only ADMIN role can access this endpoint.",
                    content = @Content(mediaType = "application/json"))
    })
    @GetMapping
    ...
    @Operation(
            ...
            description = """
                    ...
                    This endpoint allows viewing user details. Patients can only view their own
                    profile, while ADMINs and DOCTORs can view any user.

                    ## Path Parameters
                    - id: The unique identifier of the user

                    ## Response Details
                    - Returns user profile including role, tenant, and department information
                    """)
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "401", description = "Unauthorized. Invalid or missing authentication token.",
                    content = @Content(mediaType = "application/json")),
            @ApiResponse(responseCode = "403", description = "Forbidden. Patients can only view their own profile.",
                    content = @Content(mediaType = "application/json")),
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "401", description = "Unauthorized. Invalid or missing authentication token.",
                    content = @Content(mediaType = "application/json")),
            @ApiResponse(responseCode = "403", description = "Forbidden. Only ADMIN or DOCTOR roles can access.",
                    content = @Content(mediaType = "application/json")),
    ...
    public ResponseEntity<UserResponse> findByEmail(
            @Parameter(description = "The email address to search for", required = true)
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "403", description = "Forbidden. Only ADMIN or DOCTOR roles can access.",
                    content = @Content(mediaType = "application/json")),
            @ApiResponse(responseCode = "404", description = "Patient not found with specified medical record number.",
                    content = @Content(mediaType = "application/json"))
    })
    @GetMapping("/medical-record/{medicalRecordNumber}")
    @PreAuthorize("hasAnyRole('ADMIN', 'DOCTOR')")
    public ResponseEntity<UserResponse> findByMedicalRecordNumber(
            @Parameter(description = "The patient's unique medical record number", required = true)
    ...
    @Operation(
            summary = "Get Doctor by License Number",
            description = """
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "403", description = "Forbidden. Only ADMIN or DOCTOR roles can access.",
                    content = @Content(mediaType = "application/json")),
            @ApiResponse(responseCode = "404", description = "Doctor not found with specified license number.",
                    content = @Content(mediaType = "application/json"))
    })
    @GetMapping("/license/{licenseNumber}")
    @PreAuthorize("hasAnyRole('ADMIN', 'DOCTOR')")
    public ResponseEntity<UserResponse> findByLicenseNumber(
            @Parameter(description = "The doctor's professional license number", required = true)
    ...
    public ResponseEntity<Boolean> existsByEmail(
            @Parameter(description = "The email address to check for availability", required = true)
            @PathVariable String email) {
    ...
    public ResponseEntity<Void> softDelete(
            @Parameter(description = "The unique identifier of the user to delete", required = true)
            @PathVariable Long id) {
    ...
    @Operation(
            ...
            description = """
                    ...
                    """)
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Failed login recorded successfully.",
                    content = @Content(mediaType = "application/json")),
            @ApiResponse(responseCode = "404", description = "User not found.",
                    content = @Content(mediaType = "application/json"))
    })
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "403", description = "Forbidden. Only ADMIN role can access.",
                    content = @Content(mediaType = "application/json")),
            @ApiResponse(responseCode = "409", description = "Conflict. Email or medical record number already in use.",
                    content = @Content(mediaType = "application/json")),
            @ApiResponse(responseCode = "404", description = "Tenant or role not found.",
                    content = @Content(mediaType = "application/json"))
    })
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "403", description = "Forbidden. Only ADMIN role can access.",
                    content = @Content(mediaType = "application/json")),
            @ApiResponse(responseCode = "409", description = "Conflict. Email or license number already in use.",
                    content = @Content(mediaType = "application/json")),
            @ApiResponse(responseCode = "404", description = "Tenant, role, or department not found.",
                    content = @Content(mediaType = "application/json"))
    })
    @PostMapping("/doctors")
    @PreAuthorize("hasRole('ADMIN')")
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "401", description = "Unauthorized. Invalid or missing authentication token.",
                    content = @Content(mediaType = "application/json")),
            @ApiResponse(responseCode = "403", description = "Forbidden. Doctors can only update their own profile.",
                    content = @Content(mediaType = "application/json")),
    ...
    public ResponseEntity<UserResponse> updateDoctor(
            @Parameter(description = "The unique identifier of the doctor", required = true)
            @PathVariable Long id,
    ...
    public ResponseEntity<UserResponse> updateAdmin(
            @Parameter(description = "The unique identifier of the admin", required = true)
            @PathVariable Long id,
